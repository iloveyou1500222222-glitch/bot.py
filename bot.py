...
REQUIRED_CHANNEL = os.environ.get('REQUIRED_CHANNEL')  # ဥပမာ - '@BOTUAPTE'
...
# Render ပေါ်မှာ server restart ခံရရင် ဒီ data တွေ ပျက်သွားမယ်။
# အပြီးအပိုင် သိမ်းချင်ရင် Database သုံးပါ။
video_file_id = None
...
def is_user_joined_channel(user_id):
    """အသုံးပြုသူက သတ်မှတ်ထားတဲ့ Channel ကို Join ထားလား စစ်တယ်"""
    try:
...
def handle_callbacks(call):
    ...
    if not is_user_joined_channel(user_id):
        bot.answer_callback_query(
            call.id,
            f"❌ ကျေးဇူးပြု၍ {REQUIRED_CHANNEL} ကို ဦးစွာ Join လုပ်ပါ။",
            show_alert=True
        )
        return
    ...
    # ===== ၃။ "Channel Join" ခလုတ် (ပြန်စစ်ဆေးရန်) =====
        ...
        if is_user_joined_channel(user_id):
            ...
            bot.answer_callback_query(
                call.id,
                f"❌ ကျေးဇူးပြု၍ {REQUIRED_CHANNEL} ကို Join လုပ်ပါ။",
                show_alert=True
            )
...
def chat_with_ai(message):
    ...
    # Command များကို ဖယ်ရှားရန် (သို့မဟုတ် ၎င်းတို့ကို ကိုင်တွယ်ရန်)
